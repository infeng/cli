...
| [`+template-create`](references/lark-mail-templates.md) | Create a personal mail template. Auto-uploads HTML inline images and non-inline attachments to Drive (≤20MB single upload; >20MB multipart), rewrites `<img src>` to `cid:` refs, and POSTs the Template payload. |
| [`+template-update`](references/lark-mail-templates.md) | Update an existing mail template. Supports `--inspect`, `--print-patch-template`, `--patch-file` and flat `--set-*` flags. Full-replace PUT with no optimistic locking (last-write-wins). |
...
### user_mailbox.templates
  ...
  - `list` — 列出个人邮件模板（仅返回 template_id + name）
  - `get` — 获取单个模板完整字段
  - `update` — 全量替换更新（优先使用 `+template-update` Shortcut；后端无乐观锁）
  - `delete` — 删除模板（不可恢复）

### user_mailbox.template_attachments
