...
| [`+thread-modify`](references/lark-mail-thread-modify.md) | Modify existing mail threads by adding/removing label IDs or safely mapping `--folder-id` to `add_folder` in one batch request. Returns the server response unchanged. |
| [`+thread-trash`](references/lark-mail-thread-trash.md) | Soft-delete existing mail threads in one batch request and return the server response unchanged. Requires --yes. |
